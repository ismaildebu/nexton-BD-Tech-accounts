...
it('stock report running balance reflects confirmed distribution deduction', function () {
    $pub = rPub(1000);
    $party = rMParty();

    $distribution = $this->distService->create(
        $pub,
        '2026-09-01',
...
it('stock report PDF returns a PDF for a valid publication', function () {
    ...
    $response = $this->actingAs($this->admin)
        ->get(route('media.reports.stock.pdf', [
            'publication_id' => $pub->id,
        ]));
...
it('distribution summary shows confirmed distributions in date range', function () {
    ...
    $response = $this->actingAs($this->admin)->get(
        route('media.reports.distribution-summary', [
...
it('distribution summary PDF downloads correctly', function () {
    ...
    $response = $this->actingAs($this->admin)->get(
        route('media.reports.distribution-summary.pdf', [
            'from_date' => '2026-09-01',
        ])
    );
...
it('return summary shows confirmed returns', function () {
    ...
    /*
     * Return reports only confirmed returns.
     * Confirm the return before querying the report.
     */
   $return = $this->retService->create(
    $pub,
    '2026-09-02',
...
$response = $this->actingAs($this->admin)->get(
    route('media.reports.return-summary', [
        'from_date' => '2026-09-01',
    ])
);
...
it('collection summary shows collections in date range', function () {
    ...
    $response = $this->actingAs($this->admin)->get(
        route('media.reports.collection-summary', [
            'from_date' => '2026-09-01',
        ])
    );
...
it('party ledger shows distribution and collection lines for the selected party', function () {
    ...
    /*
     * Party ledger includes confirmed distributions only.
     */
    $this->distService->confirm(
        $distribution,
        $this->admin->company_id,
    ...
    $response = $this->actingAs($this->admin)->get(
        route('media.reports.party-ledger', [
            'media_party_id' => $party->id,
        ])
    );
...
it('party ledger PDF downloads correctly', function () {
    ...
    $response = $this->actingAs($this->admin)->get(
        route('media.reports.party-ledger.pdf', [
            'media_party_id' => $party->id,
        ])
    );
