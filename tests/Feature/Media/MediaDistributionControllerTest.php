...
it('stores a distribution for a single party via HTTP as a draft without updating stock', function () {
    ...
    $response = $this->actingAs($admin)->post(
        route('media.distributions.store'),
        [
    ...
    expect($distribution)->not->toBeNull()
        ->and($distribution->status)->toBe(MediaDistribution::STATUS_DRAFT)
        ->and($distribution->total_quantity)->toBe(110)
        ->and((new NewspaperStockService())->balance($publication))->toBe(1000);
});

it('rejects a distribution over HTTP when stock is insufficient during confirmation and keeps the draft', function () {
    $admin = $this->makeMediaAdmin();
    session(['company_id' => $admin->company_id]);
    ...
    expect($distribution)->not->toBeNull()
        ->and($distribution->status)->toBe(MediaDistribution::STATUS_DRAFT)
        ->and((new NewspaperStockService())->balance($publication))->toBe(10);

    $response = $this->actingAs($admin)->post(
        route('media.distributions.confirm', $distribution)
    );
    ...
    expect($distribution->status)->toBe(MediaDistribution::STATUS_DRAFT)
        ->and((new NewspaperStockService())->balance($publication))->toBe(10)
        ->and(MediaDistribution::count())->toBe(1);
});
...
it('cannot be created against another company\'s publication or party (company isolation)', function () {
    ...
    $otherCompany = $this->makeMediaCompany([
        'company_name' => 'Other Media House',
    ]);

    session(['company_id' => $otherCompany->id]);

    $foreignPublication = Publication::create([
        'name' => 'Foreign Paper',
        'code' => 'FP',
        'selling_price' => 10,
    ]);

    $foreignParty = MediaParty::create([
        'name' => 'Foreign Agent',
        'type' => 'agent',
        'code' => 'FA-1',
    ]);
...
it('downloads a dispatch sheet PDF for a confirmed distribution', function () {
    ...
    $response = $this->actingAs($admin)->get(
        route('media.distributions.dispatch-sheet', $distribution)
    );
...
it('downloads a bundle-slips PDF containing one slip per item', function () {
    ...
    $this->actingAs($admin)->post(
        ...
        [
            ...
            'items' => [
                [
                    'media_party_id' => $partyOne->id,
                    'paid_quantity' => 50,
                    'rate' => 5,
                ],
                [
                    'media_party_id' => $partyTwo->id,
                    'paid_quantity' => 30,
                    'rate' => 5,
                ],
            ],
    ...
    $response = $this->actingAs($admin)->get(
        route('media.distributions.bundle-slips', $distribution)
    );
