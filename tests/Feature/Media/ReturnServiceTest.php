...
it('records a standalone return and adds stock back', function () {
    ...
    // A standalone return is physical-only; paid returns require the original distribution.
    $return = $this->service->create(
        $pub, '2026-09-02', $this->company->id, $this->user->id,
        [['media_party_id' => $party->id, 'paid_return_quantity' => 0, 'free_return_quantity' => 30]],
    );
    ...
    expect(fn () => $this->service->create(
        ...
        it('handles a multi-party return in one document', function () {
            ...
            expect($return->items)->toHaveCount(2)
                ...
                ->and($this->stock->balance($pub))->toBe(1000 - 15 + 18);
        });
