...
class StoreLegalDocumentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            // ✅ Fix: company-scoped uniqueness — migration-এও composite
            // unique(company_id, document_number) যোগ হয়েছে।
            'document_number' => [
                'required',
                'string',
                Rule::unique('legal_documents', 'document_number')->where(fn ($q) => $q->where('company_id', $companyId)),
            ],
