        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                ...
                <div class="bg-white rounded-lg shadow-md mb-8 overflow-hidden">
                    @php
                        $fileType = strtolower($legalDocument->file_type);
                    @endphp
                    <div class="bg-gray-100 h-96 flex items-center justify-center">
                       
                    
                    @if($previewUrl && in_array($fileType, ['jpg', 'jpeg', 'png']))
                            <img
                                src="{{ $previewUrl }}"
                                alt="{{ $legalDocument->title }}"
                                class="w-full h-full object-contain"
                            >
                        @elseif($previewUrl && strtolower($legalDocument->file_type) === 'pdf')
                            <img src="{{ $previewUrl }}" alt="{{ $legalDocument->title }}" class="w-full h-full object-contain">
                        
                            @elseif($previewUrl && $fileType === 'pdf')
