...
@section('page-title', 'Edit ' . ucfirst($party->type))
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <form method="POST" action="{{ route('media.parties.update', $party) }}" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $party->code) }}" required
                           class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="alternate_phone" value="{{ old('alternate_phone', $party->alternate_phone) }}"
                           class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="address" rows="2"
                          class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('address', $party->address) }}</textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Area</label>
                    <input type="text" name="area" value="{{ old('area', $party->area) }}"
                           class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" min="0" max="100" name="free_percentage" value="{{ old('free_percentage', $party->free_percentage) }}"
                           class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="opening_balance" value="{{ old('opening_balance', $party->opening_balance) }}"
                           class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    ...
                    <select name="balance_type"
                            class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="Receivable" {{ old('balance_type', $party->balance_type) === 'Receivable' ? 'selected' : '' }}>Receivable</option>
                        <option value="Payable" {{ old('balance_type', $party->balance_type) === 'Payable' ? 'selected' : '' }}>Payable</option>
                        <option value="Advance" {{ old('balance_type', $party->balance_type) === 'Advance' ? 'selected' : '' }}>Advance</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('media.parties.show', $party) }}"
