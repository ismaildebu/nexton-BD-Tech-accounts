...
<div class="min-h-screen bg-gray-50 dark:bg-teal-950 py-6 px-4 sm:px-6 lg:px-8">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-teal-900 dark:text-white">Edit User</h1>
            <p class="text-sm text-teal-600 dark:text-teal-300 mt-1">
                Update details for <span class="font-semibold">{{ $user->name }}</span>.
            </p>
        </div>
        ...
        <div class="rounded-xl border border-teal-100 bg-white p-6 shadow-sm dark:border-teal-800 dark:bg-teal-900">
            <form action="{{ route('system.users.update', $user) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="password" id="password" name="password"
                               ...
                               class="mt-1.5 block w-full rounded-lg border border-teal-200 bg-white px-3.5 py-2.5 text-sm
                                      ...
                                      dark:border-teal-700 dark:bg-teal-950 dark:text-white dark:placeholder:text-teal-500">
                        @error('password')
                            <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                {{-- Company — only Super Admin can move a user between companies --}}
